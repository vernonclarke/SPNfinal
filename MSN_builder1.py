...
#         self._set_nsegs(section=section)
        ...
        if variables: # currently not using variables
            self.distribute_channels("dend", "gbar_naf", 1,   1.0-variables['naf'][1], variables['naf'][1], variables['naf'][2], variables['naf'][3], np.power(10,variables['naf'][0])*float(par['gbar_naf_basal']['Value']))
            self.distribute_channels("dend", "gbar_kaf", 1,   1.0, variables['kaf'][1], variables['kaf'][2], variables['kaf'][3], np.power(10,variables['kaf'][0])*float(par['gbar_kaf_basal']['Value']))
            self.distribute_channels("dend", "gbar_kas", 1,   0.1, 0.9, variables['kas'][1], variables['kas'][2], np.power(10,variables['kas'][0])*float(par['gbar_kas_basal']['Value']))
        ...
        self._set_nsegs2(freq=1000, d_lambda=0.1)
...
def add_spines(cell, spine_per_length): 
    SPINES = {}
    ID = 0
