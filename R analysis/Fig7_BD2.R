...
# Note forgot to simulate g_gaba = 0 therefore get from equivalent simulation (ie 320)
wd1 <- sub("sim320a/$", "sim320/", wd)
setwd(wd1)
...
if (plotsave) {	
	...
	setwd(save_dir)
	quartz.save(paste0('sim320a_a ', gsub(':', '-', Sys.time()), '.pdf'), type='pdf')
	dev.off()
	setwd(wd)
}
...
if (plotsave) {	
	save_dir <- str_replace(wd, 'simulations', 'images')
	setwd(save_dir)
	svglite(paste0('sim320a_a1 ', gsub(':', '-', Sys.time()), '.svg'), width=3.5 ,height=3.75, pointsize=10)
	funplot1(Vdend_GABA0, Vdend.plot, y.lim2, main='dendritic voltage', ab1=-30, ab2=-85)
	dev.off()
	svglite(paste0('sim320a_a2 ', gsub(':', '-', Sys.time()), '.svg'), width=3.5 ,height=3.75, pointsize=10)
	funplot1(Vsoma_GABA0, Vsoma.plot, y.lim1, main='somatic voltage', ab1=-60, ab2=-85)
	dev.off()
	setwd(wd)
}
...
if (plotsave) {	
	save_dir <- str_replace(wd, 'simulations', 'images')
	setwd(save_dir)
	quartz.save(paste0('sim320a_b1 ', gsub(':', '-', Sys.time()), '.pdf'), type='pdf')
	dev.off()
	setwd(wd)
}
...
if (plotsave) {	
	save_dir <- str_replace(wd, 'simulations', 'images')
	setwd(save_dir)
	quartz.save(paste0('sim320a_b2 ', gsub(':', '-', Sys.time()), '.pdf'), type='pdf')
	dev.off()
	setwd(wd)
}
...
if (plotsave) {	
	...
	svglite(paste0('sim320a_b3 ', gsub(':', '-', Sys.time()), '.svg'), width=3.625 ,height=2.875, pointsize=10)
	funplot2(X=X, Y=Y2, y.lim=y.lim2, y.int=y.int2, x.lim=x.lim, main='dendritic depolarisation (mV)')
	dev.off()

	svglite(paste0('sim320a_b4 ', gsub(':', '-', Sys.time()), '.svg'), width=3.625 ,height=2.875, pointsize=10)
	funplot2(X=X, Y=Y3, y.lim=y.lim3, x.lim=x.lim, y.int=y.int3, main='dendritic P3/P1')
	dev.off()
