...
# Note forgot to simulate g_gaba = 0 therefore get from equivalent simulation (ie 320f)
wd1 <- sub("sim320d/$", "sim320f/", wd)
setwd(wd1)
# getting control values from previous sim making Fig7 B and C
V_soma1 <- pd$read_pickle('v_data.pickle')
V_dend1 <- pd$read_pickle('vdend_data.pickle')
Vsoma1 <- sapply(1:length(V_soma1), function(ii) V_soma1[[ii]])
Vdend1 <- sapply(1:length(V_dend1), function(ii) V_dend1[[ii]])
# isolate 0 GABA and 0 Glut traces
Vsoma_GABA0 <- Vsoma1[,1]
Vdend_GABA0 <- Vdend1[,1]

Vsoma = cbind(Vsoma_GABA0, Vsoma)
Vdend = cbind(Vdend_GABA0, Vdend)
gt <- c(pd$read_pickle('gt.pickle')[[1]][[1]],gt)
...
# Clean environment
to_keep <- c('imp', 'g_GABA', 'i_GABA', 't', 'gt', 'Vdend', 'Vsoma', 'Vdend_GABA0', 'Vsoma_GABA0', 'nGlut', 'wd', 'plotsave', 'names', 'sim')
...
if (plotsave) {	
	...
	setwd(save_dir)
	quartz.save(paste0('sim320d_a ', gsub(':', '-', Sys.time()), '.pdf'), type='pdf')
	dev.off()
	setwd(wd)
}
...
if (plotsave) {	
	save_dir <- str_replace(wd, 'simulations', 'images')
	setwd(save_dir)
	svglite(paste0('sim320d_a1 ', gsub(':', '-', Sys.time()), '.svg'), width=3.5 ,height=3.75, pointsize=10)
	funplot1(Vdend_GABA0, Vdend.plot, y.lim2, main='dendritic voltage', ab1=-30, ab2=-85)
	dev.off()
	svglite(paste0('sim320d_a2 ', gsub(':', '-', Sys.time()), '.svg'), width=3.5 ,height=3.75, pointsize=10)
	funplot1(Vsoma_GABA0, Vsoma.plot, y.lim1, main='somatic voltage', ab1=-60, ab2=-85)
	dev.off()
	setwd(wd)
}
...
g_gaba <- 10^seq(log10(1e-6), log10(3e-2), length.out = 100)
g_gaba <- c(0, g_gaba)
...
funplot2 = function(X,Y, y.lim, x.lim, y.int, color_range='black', main=''){
	plot(X, Y, log="x", xlim=x.lim, ylim=y.lim, xlab='',ylab='', bty="n",  type="l", col=color_range, axes=FALSE, lwd=lwd, frame=FALSE)
...
if (plotsave) {	
	save_dir <- str_replace(wd, 'simulations', 'images')
	setwd(save_dir)
	quartz.save(paste0('sim320d_b1 ', gsub(':', '-', Sys.time()), '.pdf'), type='pdf')
	dev.off()
	setwd(wd)
}
...
if (plotsave) {	
	save_dir <- str_replace(wd, 'simulations', 'images')
	setwd(save_dir)
	quartz.save(paste0('sim320d_b2 ', gsub(':', '-', Sys.time()), '.pdf'), type='pdf')
	dev.off()
	setwd(wd)
}
...
if (plotsave) {	
	...
	svglite(paste0('sim320d_b3 ', gsub(':', '-', Sys.time()), '.svg'), width=3.625 ,height=2.875, pointsize=10)
	funplot2(X=X, Y=Y2, y.lim=y.lim2, y.int=y.int2, x.lim=x.lim, main='dendritic depolarisation (mV)')
	dev.off()

	svglite(paste0('sim320d_b4 ', gsub(':', '-', Sys.time()), '.svg'), width=3.625 ,height=2.875, pointsize=10)
	funplot2(X=X, Y=Y3, y.lim=y.lim3, x.lim=x.lim, y.int=y.int3, main='dendritic P3/P1')
	dev.off()
